...
print "\n random forests are disabled due to instability in this analysis\n\n";
sleep(3);
...
if ($method_bnp == 1){
 ...
 for (my $p = 1; $p<=$framefactor; $p++){
     $add = ($p*$framegroups-$framegroups)+4;
     $total = ($framefactor*$framegroups) + ($framefactor*4);
 for (my $r = 0; $r<$lengthID; $r++){
   ...
   # read data into R
   print Rinput "dataT = read.table('indAAtrain_flux+corr/fluxcorrAA_$refID"."_$r.txt', header = TRUE)\n";
   print Rinput "dataD = read.table('testingData_$fileIDq/indAAtest_flux+corr/fluxcorrAA_$fileIDq"."_$r.txt', header = TRUE)\n";
...
###########################################
if ($method_ens == 1 && $method_ens == 0){  # random forest is disabled
#if ($method_ens == 1){  # random forest is enabled    
