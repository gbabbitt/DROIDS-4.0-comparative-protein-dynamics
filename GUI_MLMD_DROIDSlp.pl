...
$cutoffValueProd = $cutoffValueProdFS/1000000;
...
#				);
	...
	my $dforceFrame = $pdbFrame->Frame();
		my $dforceLabel = $dforceFrame->Label(-text=>"additional small molecule force field: ");
...
my $antechamberButton = $mw -> Button(-text => "ligand force field modification (skip if not needed)", 
				-command => \&antechamber
...
ADD_Field\t$dforceID\t# AMBER force field to use in MD runs
...
system "perl teLeap_ligandproteinQuery_deploy.pl\n";
...
if($g == 1){system "pdb4amber -i $fileIDl.pdb -o ".$fileIDl."REDUCED.pdb --dry --reduce \n";}
close MUT;
close MUT2;
} #end for loop
} # end outer for loop
...
mkdir ("atomcorr_deploy_$fileIDq") or die "please delete atomcorr folder from previous run\n";
print "parsing atom corr data\n";
open(IN1, "<"."DROIDSfluctuationAVGchain_deploy_$fileIDq.txt") or die "could open output file\n";
@IN1 = <IN1>;
for (my $i = 0; $i < scalar @IN1; $i++){
    if ($i == 0){next;}
    print "creating file ./atomcorr_deploy_$fileIDq/DROIDScorrelation_$i.txt\n";
    open(OUT, ">"."./atomcorr_deploy_$fileIDq/DROIDScorrelation_$i.txt") or die "could not open output file\n";
    print OUT "sample\t"."pos_ref\t"."res_ref\t"."res_query\t"."atomnumber\t"."atomlabel\t"."flux_ref\t"."flux_query\t"."corr1_query\t"."corr3_query\t"."corr5_query\t"."corr9_query\n";
    open(IN2, "<"."./atomflux_deploy_$fileIDq/DROIDSfluctuation_$i.txt") or die "could open not input file\n";
    @IN2 = <IN2>;
    for (my $ii = 0; $ii < scalar @IN2; $ii++){
        ...
        open(IN4, "<"."corr_1res_$fileIDq"."_deploy.txt") or die "could open not input file\n";
            @IN4 = <IN4>;
        ...
        open(IN6, "<"."corr_3res_$fileIDq"."_deploy.txt") or die "could open not input file\n";
            @IN6 = <IN6>;
        ...
        open(IN8, "<"."corr_5res_$fileIDq"."_deploy.txt") or die "could open not input file\n";
            @IN8 = <IN8>;
        ...
        open(IN10, "<"."corr_9res_$fileIDq"."_deploy.txt") or die "could open not input file\n";
            @IN10 = <IN10>;
        ...
        print OUT "$sample\t"."$pos_ref\t"."$res_ref\t"."$res_query\t"."$atomnumber\t"."$atomlabel\t"."$flux_ref\t"."$flux_query\t"."$corr_1res_query\t"."$corr_3res_query\t"."$corr_5res_query\t"."$corr_9res_query\n";
        }
    close IN2;
    close OUT;
}
close IN1;
###################################################
##### create combined flux+corr indAAtest folder
###################################################
mkdir("./testingData_$fileIDq/indAAtest_flux+corr");
print("\nparsing testing set time series for each amino acid in $fileIDq...\n");
sleep(1);
# make labels
...
$corrlabels = "$corrlabel1\t"."$corrlabel3\t"."$corrlabel5\t"."$corrlabel9";
for (my $t = 0; $t < $lengthID; $t++){
  $tt = $t+1;   #if($t == 0){next;}  
  open(OUT, ">"."./testingData_$fileIDq/indAAtest_flux+corr/fluxcorrAA_$fileIDq"."_$t".".txt")||die "could not create AA time series file\n";
  open(IN1, "<"."./testingData_$fileIDq/indAAtest/fluxtimeAA_$fileIDq"."_$t".".txt")||die "could not open flux file "."fluxtimeAA_$refID"."_$t".".txt\n";
  open(IN2, "<"."./atomcorr_deploy_$fileIDq/DROIDScorrelation"."_$tt".".txt"); # ||die "could not open corr file "."corrtimeAA_$refID"."_$tt".".txt\n";
  @IN1 = <IN1>;
  @IN2 = <IN2>;
  $corrs = "0.5\t"."0.5\t"."0.5\t"."0.5"; # initialize
  for (my $a = 0; $a < scalar @IN1; $a++) {
      ...
      chomp $IN1row;
      chomp $IN2row;
      # divide $IN1row into equal number chunks matching number framegroups
      @IN1row = split (/\s+/, $IN1row);
      my @BREAKS = ();
      for(my $g = 0; $g < $prodLen; $g++){
          $beginslice = $framegroups*$g;
          $endslice = $framegroups*($g+1)-1;
          @SLICE = @IN1row[$beginslice..$endslice];
          $slice = join("\t", @SLICE);
          push(@BREAKS, $slice);
          #print $slice;
      }
      ...
      $corr1 = $IN2row[8]; $corr3 = $IN2row[9]; $corr5 = $IN2row[10]; $corr9 = $IN2row[11];
      $corrs = "$corr1\t"."$corr3\t"."$corr5\t"."$corr9";
      #print "check corrs\n";
      #print "$corrs\n\n";
      if($corrs == ''){$corrs = "0.5\t"."0.5\t"."0.5\t"."0.5";} # avg if array is empty
      if($a == 0){
          for(my $h = 0; $h < $prodLen; $h++){
          if($h != $prodLen - 1){print OUT "$BREAKS[$h]\t"."$corrlabels\t";}
          if($h == $prodLen - 1){print OUT "$BREAKS[$h]\t"."$corrlabels";} # no tab on final iter
          }
          print OUT "\n";
          }
      if($a != 0){
           for(my $hh = 0; $hh < $prodLen; $hh++){
           if($hh != $prodLen - 1){print OUT "$BREAKS[$hh]\t"."$corrs\t";}
           if($hh == $prodLen - 1){print OUT "$BREAKS[$hh]\t"."$corrs";} # no tab on final iter
           }
           print OUT "\n";
           }
      }
