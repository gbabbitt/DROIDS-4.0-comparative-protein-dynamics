...
# create array of names of copies, orthologs, and variants
...
for (my $p = 0; $p < scalar @MUT; $p++){
	 if ($p == 0){next;}
      ...
      my @MUTrow = split (/\s+/, $MUTrow);
	 $fileIDq = $MUTrow[0];
      if ($p == 1){push (@orthologs, $fileIDq);}
      if ($p == 3){push (@orthologs, $fileIDq);}
      } #end for loop
close MUT;
print "\n orthologs are @orthologs\n\n";
sleep(1);
...
##################################################################
###  analyze copies for plots 1 and 2 (functional dynamics)
##################################################################
...
# create plot 2
print Rinput "myplot2 <- ggplot() + ggtitle('cancor for $window AA window + functional dynamics zones -Wilks lambda') + geom_area(data = dataframe4_$copyID, mapping = aes(x = pos, y = bars, color = 'functional dynamics'), alpha = 0.8) + labs(x = 'position (residue number)', y = 'local R value for cancor') + theme(legend.title = element_blank())\n"; 
...
my $newfilename = "./testingData_$queryID/learningPLOTfunctional.pdf";
copy($oldfilename, $newfilename);	
print " machine learning is complete\n\n";
print " close PDF and txt viewer to continue\n\n";
system "evince ./testingData_$queryID/learningPLOTfunctional.pdf\n";
...
########################################################
###  analyze variants for plot 3 (functional dynamics)
########################################################
...
my $newfilename2 = "./maxDemon_results/learningPLOTfunctional.pdf";
...
##############################################################################################
######################  mapping functional dynamics to PDB structure  #########################
##############################################################################################
...
sleep(1);print "\nSELECT INTERACTION TYPE FOR MAPPING CONSERVED DYNAMICS (1=protein only | 2=protein-protein | 3=DNA-protein | 4=protein-ligand)\n\n";
...
##############################################################################
###  analyze copies for plots 1 and 2 (evolutionary conserved dynamics)
##############################################################################
...
for (my $v = 0; $v < scalar(@orthologs); $v++){
     $copyID = $orthologs[$v];
     $queryID = $orthologs[0];
     $variantID_label = $variant_labels[$v];
     $queryID_label = $variant_labels[0];
...
# output stats file
print Rinput "sink('./testingData_$queryID/cancor_stats_ortholog.txt')\n";
print Rinput "cat('CANONICAL CORRELATION ANALYSIS OF $queryID and new variants\n\n')\n";
print Rinput "cat('there are 6 dimensions and 7 learners\n\n')\n";
print Rinput "cat('cor = overall correlation of all learners in each dimension\n')\n";
print Rinput "cat('coef = slope of each pairwise learner correlation for each dimension\n')\n";
...
print Rinput "print(my_adjbars_$queryID)\n";  
print Rinput "sink('./testingData_$queryID/cancor_stats_ortholog_$copyID.txt')\n";
print Rinput "cat('CANONICAL CORRELATION ANALYSIS OF $queryID and $copyID ($window AA sliding window)\n\n')\n";
print Rinput "print(my_cancors_$copyID)\n";
print Rinput "sink()\n";
print Rinput "dataframe4_$copyID = data.frame(pos=my_ccpos_$copyID+$startN, cc=my_cancors_$copyID, bars=my_adjbars_$queryID)\n";
...
print Rinput "print(my_ccor_$copyID)\n";
# create output mask movie render file for Wilks lambda
print Rinput "sink('./testingData_$queryID/adj_pvals_ortholog_$copyID.txt')\n";
print Rinput "print(my_adjbars_$queryID)\n";
print Rinput "sink()\n";
} # end for loop
...
sleep(1);
my $oldfilename = "Rplots.pdf";
my $newfilename = "./testingData_$queryID/learningPLOTortholog.pdf";
copy($oldfilename, $newfilename);	
print " machine learning is complete\n\n";
print " close PDF and txt viewer to continue\n\n";
system "evince ./testingData_$queryID/learningPLOTortholog.pdf\n";

# open can corr results
system "gedit ./testingData_$queryID/cancor_stats_ortholog.txt\n";
for (my $v = 0; $v < scalar(@orthologs); $v++){
     $copyID = $orthologs[$v];
     $queryID = $orthologs[0];
     $variantID_label = $variant_labels[$v];
     $queryID_label = $variant_labels[0];
     system "gedit ./testingData_$queryID/cancor_stats_ortholog_$copyID.txt\n";
} # end for loop  
...
########################################################
###  analyze variants for plot 3 (conserved dynamics)
########################################################
...
print Rinput "print(my_adjbars_$queryID)\n";  
print Rinput "sink('./testingData_$queryID/cancor_stats_ortholog_$variantID.txt')\n";
print Rinput "cat('CANONICAL CORRELATION ANALYSIS OF $queryID and $variantID ($window AA sliding window)\n\n')\n";
print Rinput "print(my_cancors_$variantID)\n";
print Rinput "cat('\nMUTATIONAL IMPACT ANALYSIS OF $queryID and $variantID ($window AA sliding window)\n\n')\n";
print Rinput "print(my_impact_$variantID)\n";
...
# create output impact files for movie render
print Rinput "sink('./testingData_$queryID/impact_ortholog_$variantID.txt')\n";
print Rinput "print(my_impact_$variantID)\n";
print Rinput "sink()\n";   
...
print Rinput "pval <- pf(q=fval, df1=numDF, df2=denomDF, lower.tail=FALSE)\n";
print Rinput "print(pval)\n";
print Rinput "sink('./maxDemon_results/ANOVAresult_ortholog.txt')\n";
print Rinput "library(rpsychi)\n";     
print Rinput "dataframe7 = data.frame(var = my_impact_IDs, sum = my_impact_sums, sd = mySE, n = my_impact_n)\n";
print Rinput "print(dataframe6)\n";
print Rinput "myANOVA <- with(dataframe7, ind.oneway.second(sum,sd,n))\n";
...
copy($oldfilename, $newfilename);	
my $oldfilename2 = "Rplots.pdf";
my $newfilename2 = "./maxDemon_results/learningPLOTortholog.pdf";
copy($oldfilename2, $newfilename2);
print " machine learning is complete\n\n";
print " close PDF and txt viewer to continue\n\n";
system "evince ./testingData_$queryID/learningPLOTortholog.pdf\n";
...
# open can corr results
system "gedit ./testingData_$queryID/cancor_stats_ortholog.txt\n";
     my $oldfilename3 = "./testingData_$queryID/cancor_stats_ortholog.txt";
     my $newfilename3 = "./maxDemon_results/cancor_stats_ortholog.txt";
     copy($oldfilename3, $newfilename3);
for (my $v = 0; $v < scalar(@variants); $v++){
     $variantID = $variants[$v];
     $queryID = $variants[0];
     system "gedit ./testingData_$queryID/cancor_stats_ortholog_$variantID.txt\n";
     my $oldfilename4 = "./testingData_$queryID/cancor_stats_ortholog_$variantID.txt";
     my $newfilename4 = "./maxDemon_results/cancor_stats_ortholog_$variantID.txt";
     copy($oldfilename4, $newfilename4);
} # end for loop
if(scalar(@variants) >= 4){system "gedit ./maxDemon_results/ANOVAresult_ortholog.txt\n";}
#}
...
# create mask for signif Wilks lamda
open(OUT, ">"."./testingData_$queryID/adj_vertpvals_ortholog_$queryID.txt")||die "could not create mask file /testingData_$queryID/adj_vertpvals_$queryID.txt\n";  
open(IN, "<"."./testingData_$queryID/adj_pvals_ortholog_$queryID.txt")||die "could not open mask file /testingData_$queryID/adj_pvals_$queryID.txt\n";  
my @IN = <IN>;
for (my $i = 0; $i < scalar @IN; $i++) {
	$INrow = $IN[$i];
     @INrow = split(/] /, $INrow);
...
# make learned class attribute files for image rendering
  ...
  print OUT "\n";
   ...
   for (my $a = 0; $a < $lengthID+$offset+1; $a++){
   if ($a eq '' || $a <= $offset){next;}
   open(IN, "<"."./testingData_$queryID/adj_vertpvals_ortholog_$queryID.txt")||die "could not open mask file /testingData_$queryID/adj_vertpvals_$queryID.txt\n";  
    my @IN = <IN>;
   $INrow = $IN[$a-$offset];
    @INrow = split(/\s+/, $INrow);
    $maskvalue = $INrow[0];
